...
### 注意：生成的mapper模糊查询没有对 “%”和“_”进行转义，需要修改
将id为Example_Where_Clause的sql节点中
```
...
```
<when test="criterion.singleValue" >
  <if test='criterion.condition.indexOf("like") != -1'>
	<if test='criterion.value == "%%"'>
		and ${criterion.condition} #{criterion.value}
	</if>
	<if test='criterion.value != "%%"'>
        and ${criterion.condition} concat('%',replace(replace(substring(#{criterion.value},2,char_length(#{criterion.value})-2), '%', '/%'),'_','/_'),'%') escape  '/'
        </if>
  </if>
  <if test='criterion.condition.indexOf("like") == -1'>
	 and ${criterion.condition} #{criterion.value}
